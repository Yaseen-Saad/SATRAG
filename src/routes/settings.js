...
router.post('/changeLName', requireAuth, async (req, res) => {
    try { await settingsEngine.changeLastName(req.user, req.body.lastName) } catch (err) { console.error('Last name error:', err) }
    res.redirect('/settings')
})
router.post('/changeGrade', requireAuth, async (req, res) => {
    try { await settingsEngine.changeGradeLevel(req.user, req.body.grade) } catch (err) { console.error('Grade error:', err) }
    res.redirect('/settings')
})
router.post('/changeLLMkey', requireAuth, async (req, res) => {
    try { await settingsEngine.updateLLMAPIKey(req.user, req.body.llmKey) } catch (err) { console.error('LLM key error:', err) }
    res.redirect('/settings')
})
router.post('/changeEmbeddingkey', requireAuth, async (req, res) => {
    try { await settingsEngine.updateEmbeddingsAPIKey(req.user, req.body.embeddingKey) } catch (err) { console.error('Embedding key error:', err) }
    res.redirect('/settings')
})
router.post('/changeLeaderboardStatus', requireAuth, async (req, res) => {
    ...
    } catch (err) { console.error('Leaderboard toggle error:', err) }
    res.redirect('/settings')
})
